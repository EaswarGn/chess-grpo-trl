from tqdm import tqdm
from datasets import load_dataset, Dataset, DatasetDict
from prompts import system_prompt, build_user_prompt, build_completion


def process_split(dataset, split_name, color="#00ff00"):
    """
    Processes a dataset split by appending prompt/completion columns to the existing data.
    """
    processed_data = []
    
    for example in tqdm(dataset, desc=f"Processing {split_name} split", colour=color):
        # 1. Construct the new columns
        prompt = [
            {"content": system_prompt, "role": "system"},
            {"content": build_user_prompt(example["fen"]), "role": "user"}
        ]
        
        # 2. Append the new data to the existing dictionary
        # This keeps 'fen', 'answer', 'first_move', and any other original columns
        example["prompt"] = prompt
        
        processed_data.append(example)
    
    # 3. Convert the list of updated dicts back to a Dataset
    return Dataset.from_list(processed_data)


def process_dataset(dataset_id: str = "codingmonster1234/chess-puzzles-rlvr") -> DatasetDict:
    # 1. Load the raw datasets
    print("Loading source datasets...")
    raw_dataset = load_dataset(dataset_id)
    
    processed_splits = {}
    
    for split in raw_dataset:
        # 2. Process each split into HF Dataset objects
        processed_split = process_split(raw_dataset[split], split_name=split, color="#2ecc71")
        processed_splits[split] = processed_split

    # 3. Combine into a DatasetDict to preserve split names
    dataset_dict = DatasetDict(processed_splits)
    return dataset_dict